...
//	Description 	：	KComVariant class and KFakeCopyVaraint class
//
// -----------------------------------------------------------------------*/
...
public:
	...
	KComVariant(IDispatch* pSrc)
	{
		...
		if (V_DISPATCH(this) != NULL) // Need to AddRef as Clear() will Release
	...
	KComVariant(IUnknown* pSrc)
	{
		...
		V_UNKNOWN(this) = pSrc;
		if (V_UNKNOWN(this) != NULL) // Need to AddRef as Clear() will Release
...
public:
	...
	HRESULT ChangeType(VARTYPE vtNew, const VARIANT* pSrc = NULL)
	{
		...
		if (pVar == NULL) // Convert in place if pSrc is NULL
			pVar = this;
...
public:
	bool operator==(const VARIANT& varSrc) const
	{
		...
		if (V_VT(this) != V_VT(&varSrc)) // Variants not equal if types don't match
			return false;

		switch (V_VT(this))	// Check type specific values
		{
		case VT_EMPTY:
		case VT_NULL:
			return true;
...
// -------------------------------------------------------------------------
// --> KFakeCopyVaraint unsupport reference count, string copy, array copy.
